...
def home(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and form1.is_valid():
            ...
            if "pulse" in request.POST  :
                ...
                if "plotFirstAndLastPulse" in request.POST:
                    plotFirstAndLastPulse_fig = func_plotFirstAndLastPulse_fig(testPulse0,testPulseFinal,fiber,sim_config)
                    context = {"form":form, "form1":form1,"form3":form3,"plotFirstAndLastPulse_fig":plotFirstAndLastPulse_fig}
                    ...
                    gif_file_path = 'projet2a/static/images/pulse_animation.gif'
                    ...
                    points = np.array( [sim_config.t*1e12 ,  getPower(pulseMatrix[fiber.ntraces-1,:])   ]  ).T.reshape(-1,1,2)
                    ...
                    lc.set_array( getChirp(sim_config.t,pulseMatrix[fiber.ntraces-1,:])/1e9 )
                    ...
                    def update(i):
                        ...
                        points = np.array( [sim_config.t*1e12 ,  getPower(pulseMatrix[i,:])   ]  ).T.reshape(-1,1,2)
                        ...
                        lc.set_array( getChirp(sim_config.t,pulseMatrix[i,:])/1e9 )
                        
                        #Plot line
                        line = ax.add_collection(lc)

                    #Make animation
                    ani = FuncAnimation(fig,update,range(fiber.ntraces),init_func=init)
    ...
    return render(request, "projet2a/home.html",context)
...
def func_plotFirstAndLastPulse_fig(testPulse0,testPulseFinal, fiber, sim):
    ...
    #initial_pulse = getPower(matrix[0, int(sim.number_of_points/2-nrange):int(sim.number_of_points/2+nrange)])
    #final_pulse = getPower(matrix[-1, int(sim.number_of_points/2-nrange):int(sim.number_of_points/2+nrange)])
    t = sim.t * 1e12
